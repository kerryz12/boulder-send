      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((testimonial, index) => (
            <div
              key={index}
              className="bg-slate-50 border border-slate-200/80 rounded-xl p-8 relative shadow-xl flex flex-col"
            >
              <div className="absolute top-6 right-7 text-blue-500/20 text-8xl font-serif opacity-60 transform -translate-y-3 pointer-events-none">
                “
              </div>
              <p className="text-slate-600 mb-8 relative z-10 leading-relaxed text-md flex-grow">
                {" "}
                {testimonial.quote}
              </p>
        ...
        <div className="mt-20 grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-10">
          {[
            ...
            <div
              key={stat.label}
              className="text-center p-4 bg-slate-100/70 rounded-lg shadow-md border border-slate-200/60"
            >
              <div className="text-3xl md:text-4xl font-extrabold text-blue-500 mb-1.5">
                {stat.count}
              </div>
